...
import { contactSchema, SUBJECT_SUGGESTIONS } from "@/types/contact";
...
export default function Contact({ settings }: Props) {
  ...
  const whatsapp   = getSetting(settings, "whatsapp",   "+251959155555");
  const telegram   = getSetting(settings, "telegram",   "+251959155555");
  ...
  const mapsUrl    = getSetting(settings, "maps_url",   "https://www.google.com/maps/place/Awlo+Business+center/@9.02497,38.74689,17z");
  const mapsEmbed  = getSetting(settings, "maps_embed", "https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d982.4522739821397!2d38.74689!3d9.02497!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x0%3A0xa2de1724cdb233da!2sAwlo%20Business%20center!5e0!3m2!1sen!2set!4v1720000000000");
  ...
  const isBusy       = isSubmitting || apiState === "loading";
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <p className="text-gray-500 text-lg max-w-2xl mx-auto leading-relaxed font-light">
            Have a question or ready to advertise? Send us a message and we&apos;ll get back to you within 24 hours.
          </p>
        </motion.div>
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-start">
          ...
          >
            <h3 className="font-heading font-bold text-xl text-gray-900 mb-6">Reach Us Directly</h3>
              ...
              {CONTACT_INFO.map((info, i) => {
                ...
                return (
                  <motion.a
                    ...
                    whileTap={{ scale: 0.99 }}
                    className="flex items-center gap-4 p-4 rounded-2xl glass-panel border border-white/60 hover:border-[#0057D9]/25 hover:shadow-md transition-all duration-300 group"
                  >
                    <div
                    ...
                    <div>
                      <div className="text-xs text-gray-400 font-semibold uppercase tracking-wider mb-0.5">{info.label}</div>
                      <div className="font-semibold text-gray-800 text-sm group-hover:text-[#0057D9] transition-colors">{info.value}</div>
                    </div>
                  </motion.a>
                );
              })}
          ...
          >
            <div className="glass-panel rounded-3xl border border-white/60 shadow-xl p-6 sm:p-8">
              <h3 className="font-heading font-bold text-xl text-gray-900 mb-6">Send Us a Message</h3>

              <AnimatePresence mode="wait">
                {apiState === "success" && (
                  <motion.div key="success" initial={{ opacity: 0, y: -8, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.3 }}
                  ...
                  >
                    ...
                    <div>
                      ...
                      <div className="text-xs text-emerald-600 mt-0.5">{apiMessage || "We'll get back to you within 24 hours."}</div>
                    </div>
                  </motion.div>
                )}
                {apiState === "error" && (
                  <motion.div key="error" initial={{ opacity: 0, y: -8, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.3 }}
                    className="flex items-start gap-3 bg-red-50 border border-red-200 text-red-700 rounded-2xl px-5 py-4 mb-6"
                  >
                    <AlertCircle size={20} className="flex-shrink-0 mt-0.5" />
                    <div>
                      <div className="font-bold text-sm">Failed to send message.</div>
                      <div className="text-xs text-red-600 mt-0.5">{apiMessage || "Please try again or contact us directly."}</div>
                    </div>
                  </motion.div>
                )}
              </AnimatePresence>
              ...
              <form onSubmit={handleSubmit(onSubmit)} noValidate className="space-y-5">
                <div className="grid sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    <label htmlFor="email" className="block text-sm font-semibold text-gray-700 mb-1.5">Email Address <span className="text-[#0057D9]">*</span></label>
                    <div className="relative">
                      <Mail size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" />
                      <input {...register("email")} id="email" type="email" placeholder="you@company.com" autoComplete="email" className={`${inputCn(!!errors.email)} pl-10`} />
                    </div>
                ...
                <div>
                  <label htmlFor="phone" className="block text-sm font-semibold text-gray-700 mb-1.5">Phone Number <span className="text-gray-400 font-normal">(optional)</span></label>
                  <div className="relative">
                    <Phone size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" />
                    <input {...register("phone")} id="phone" type="tel" placeholder={phone} autoComplete="tel" className={`${inputCn(!!errors.phone)} pl-10`} />
                  </div>
                  {errors.phone && <p className="text-red-500 text-xs mt-1 font-medium">{errors.phone.message}</p>}
                </div>

                <div>
                  <label htmlFor="subject" className="block text-sm font-semibold text-gray-700 mb-1.5">Subject <span className="text-[#0057D9]">*</span></label>
                  <div className="relative">
                    <Tag size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" />
                    <input {...register("subject")} id="subject" type="text" placeholder="e.g. Advertising Package Inquiry" className={`${inputCn(!!errors.subject)} pl-10`} />
                  </div>
                  {errors.subject && <p className="text-red-500 text-xs mt-1 font-medium">{errors.subject.message}</p>}
                  <div className="flex flex-wrap gap-1.5 mt-2">
                    {SUBJECT_SUGGESTIONS.map((s) => (
                      <motion.button key={s} type="button" whileTap={{ scale: 0.96 }}
                        onClick={() => setValue("subject", s, { shouldValidate: true })}
                        className={`text-xs px-3 py-1 rounded-full border transition-all duration-150 font-medium ${subjectValue === s ? "bg-[#0057D9] text-white border-[#0057D9]" : "bg-white text-gray-500 border-gray-200 hover:border-[#0057D9] hover:text-[#0057D9]"}`}
                      >{s}</motion.button>
                    ))}
                  </div>
                </div>
                ...
                <div>
                  <div className="flex items-center justify-between mb-1.5">
                    <label htmlFor="message" className="block text-sm font-semibold text-gray-700">Message <span className="text-[#0057D9]">*</span></label>
                    <span className={`text-xs font-medium ${messageValue.length > 1800 ? "text-red-400" : "text-gray-400"}`}>{messageValue.length}/2000</span>
                  </div>
                  <div className="relative">
                ...
                <p className="text-center text-xs text-gray-400">You&apos;ll receive a confirmation email. We respond within 24 hours on business days.</p>
              </form>
            </div>
          </motion.div>
        </div>
