...
export default function Packages({ settings }: Props) {
  ...
  const packages = [
    {
      duration: "1 Week",
      badge: null,
      price:   getSetting(settings, "pkg_1week_price",    "ETB 47,036"),
      tagline: getSetting(settings, "pkg_1week_tagline",  "Try it out"),
      features: [adDuration, playsPerDay, "280 total displays", "Basic support"],
      cta: "Get Started", popular: false,
    },
    {
      duration: "1 Month",
      badge: null,
      price:   getSetting(settings, "pkg_1month_price",   "ETB 108,460"),
      tagline: getSetting(settings, "pkg_1month_tagline", "Most popular for starters"),
      features: [adDuration, playsPerDay, "1,200 total displays", "Priority support", "Ad review assistance"],
      cta: "Get Started", popular: false,
    },
    {
      duration: "3 Months",
      badge: "Best Value",
      price:   getSetting(settings, "pkg_3months_price",   "ETB 291,500"),
      tagline: getSetting(settings, "pkg_3months_tagline", "Build your brand presence"),
      features: [adDuration, playsPerDay, "3,600 total displays", "Priority support", "Ad review assistance", "Campaign performance update"],
      cta: "Get Started", popular: true,
    },
    {
      duration: "6 Months",
      badge: null,
      price:   getSetting(settings, "pkg_6months_price",   "ETB 379,500"),
      tagline: getSetting(settings, "pkg_6months_tagline", "Serious brand exposure"),
      features: [adDuration, playsPerDay, "7,200 total displays", "Dedicated support", "Ad review assistance", "Campaign performance updates"],
      cta: "Get Started", popular: false,
    },
    {
      duration: "1 Year",
      badge: "Max Savings",
      price:   getSetting(settings, "pkg_1year_price",   "ETB 726,000"),
      tagline: getSetting(settings, "pkg_1year_tagline", "Dominate your market"),
      features: [adDuration, playsPerDay, "14,600 total displays", "VIP dedicated support", "Ad review assistance", "Campaign performance updates", "Seasonal creative refresh"],
            ...
            >
              ...
              >
                {pkg.badge && (
                  <motion.div
                    ...
                    className={`absolute top-4 right-4 flex items-center gap-1 text-[10px] font-extrabold px-3 py-1.5 rounded-full ${
                      pkg.popular ? "bg-[#FFD400] text-gray-900 shadow-md" : "bg-[#0057D9] text-white"
                    }`}
                  >
                ...
                <div className="p-6 flex flex-col flex-1">
                  <div className={`font-heading font-extrabold text-2xl mb-1 tracking-tight ${pkg.popular ? "text-white" : "text-gray-900"}`}>
                    {pkg.duration}
                  </div>
                  <div className={`text-xs mb-6 font-medium ${pkg.popular ? "text-white/60" : "text-gray-500"}`}>
                    {pkg.tagline}
                  </div>

                  <div className={`mb-6 py-4 px-4 rounded-2xl text-center border transition-colors duration-300 ${
                    pkg.popular ? "bg-white/10 border-white/10" : "bg-gray-50/80 border-gray-200/60"
                  }`}>
                    <div className={`font-heading font-extrabold text-2xl tracking-tight ${pkg.popular ? "text-[#FFD400]" : "text-[#0057D9]"}`}>
                      {pkg.price}
                    </div>
                    <div className={`text-[10px] mt-1 font-medium ${pkg.popular ? "text-white/40" : "text-gray-500"}`}>
                      {vatNote}
                    </div>
                  </div>
                  ...
                  <ul className="space-y-3.5 flex-1 mb-8">
                    {pkg.features.map((f) => (
                      <li key={f} className="flex items-start gap-2.5">
                        <div className={`w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 ${pkg.popular ? "bg-[#FFD400]/25" : "bg-[#0057D9]/10"}`}>
                          <Check size={11} className={pkg.popular ? "text-[#FFD400]" : "text-[#0057D9]"} strokeWidth={3} />
                        </div>
                        <span className={`text-sm leading-snug font-light ${pkg.popular ? "text-white/90" : "text-gray-700"}`}>{f}</span>
                      </li>
                    ))}
                  </ul>
